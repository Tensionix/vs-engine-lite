...
def write_notices(records: list[dict], notice_path: Path) -> None:
    ...
    for record in records:
        ...
        lines.append(
            f"| {markdown_table_cell(record.get('name', ''))} | "
            f"{markdown_table_cell(record.get('version', ''))} | "
            f"{markdown_table_cell(record.get('license', 'UNKNOWN'))} | "
            f"{markdown_table_cell(record.get('source', ''))} | {files_cell} |"
        )
    lines.extend([
        '',
        '## Policy',
