...
PRESERVE_ROOT_FILES = {'.gitkeep', 'RELEASE_CHECKLIST.md', 'RELEASE_POLICY.md', 'VS_PLUGIN_LICENSE_REVIEW.md'}
...
def find_default_site_package_dirs(root: Path) -> list[Path]:
    candidates = [
        root / 'runtime' / 'Lib' / 'site-packages',
        root / 'system_core' / 'vapoursynth' / 'Lib' / 'site-packages',
        root / 'python' / 'Lib' / 'site-packages',
        root / '.venv' / 'Lib' / 'site-packages',
        root / 'venv' / 'Lib' / 'site-packages',
        root / 'runtime' / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages',
        root / 'system_core' / 'vapoursynth' / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages',
...
def find_default_site_packages(root: Path) -> Path | None:
    dirs = find_default_site_package_dirs(root)
...
def write_notices(records: list[Record], notice_path: Path, project_root: Path) -> None:
    ...
    for record in records:
        ...
        lines.append(
            f'| {markdown_table_cell(record.name)} | '
            f'{markdown_table_cell(record.version)} | '
            f'{markdown_table_cell(record.license)} | '
            f'{markdown_table_cell(record.source)} | {files_cell} |'
        )
    lines.extend([
        '',
        '## Policy',
        '',
        '- Add missing baseline notices for PowerShell, Python Embedded, 7-Zip, fzf, and NiceGUI when they are present in the final release package.',
...
def main() -> int:
    ...
    site_packages_dirs = [Path(args.site_packages_dir).resolve()] if args.site_packages_dir else find_default_site_package_dirs(project_root)
